...
async function downloadToBuffer(url) {
    ...
    let ext = '.mp4';
    if (ct.includes('quicktime')) ext = '.mov';
    else if (ct.includes('webm')) ext = '.webm';
    return { buffer: Buffer.from(res.data), ext, mime: ct.split(';')[0].trim() || 'video/mp4' };
}

async function enhance(videoUrl) {
    ...
    const { buffer, ext, mime } = await downloadToBuffer(videoUrl);
    const filename = `video_${Date.now()}${ext}`;

    const signParams = baseParams(gnum, { suffix: '.mp4', type: 'temp', count: '1' });
    ...
    const videoInfoBody = baseParams(gnum, { file_key: fileKey });
    await api.post('/api/file/video_cover_and_display_info_ext.json', videoInfoBody.toString(), {
        headers: { ...traceHeaders(), 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8' }
    });

    const transcodeBody = baseParams(gnum, { file_key: fileKey });
    const transcodeRes = await api.post('/api/file/video_trans_start.json', transcodeBody.toString(), {
        headers: { ...traceHeaders(), 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8' }
    });
    if (transcodeRes.status >= 400 || transcodeRes.data?.code !== 0 || !transcodeRes.data?.data?.id) {
        throw new Error(`transcode start gagal: ${JSON.stringify(transcodeRes.data)}`);
    }
    const transcodeId = transcodeRes.data.data.id;

    let videoTranscoded = sourceUrl;
    for (let i = 0; i < 80; i++) {
        const qParams = baseParams(gnum, { id: transcodeId });
        const qRes = await api.get(`/api/file/video_trans_query.json?${qParams.toString()}`, { headers: traceHeaders() });
        if (qRes.status >= 400 || qRes.data?.code !== 0) throw new Error(`transcode query gagal: ${JSON.stringify(qRes.data)}`);
        const d = qRes.data.data;
        const transcoded = d?.video_transcoded || d?.transcoded_video || d?.transcoded_url || d?.video_url || '';
        if (transcoded) { videoTranscoded = transcoded; break; }
        await sleep(3000);
    }
    ...
    const deliveryBody = baseParams(gnum, {
        ...
        right_detail: JSON.stringify({ source: '1', touch_type: '4', function_id: '630', material_id: '63011', url: `${BASE_URL}/video-enhancer/upload` }),
    ...
    for (let i = 0; i < 120; i++) {
        ...
        const resultUrl = item?.result?.media_info_list?.[0]?.media_data || item?.result?.result_url || item?.result?.url || item?.client_ext_params?.video_transcoded || '';
...
module.exports = (app) => {
    app.get('/tools/winkvid', async (req, res) => {
        const { url } = req.query;

        if (!url) return res.status(400).json({
            status: false,
            message: "Parameter 'url' wajib diisi! Contoh: /tools/winkvid?url=https://example.com/video.mp4"
        });
